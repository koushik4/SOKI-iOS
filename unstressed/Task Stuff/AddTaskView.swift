    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                TextField("Assign to", text: $task.assigneeFirstName)
                    .padding()
                ...
                SokiButton(title: "Add Task", height: 45, width: 330, color: Constants.BTN_BG) {
                    task.addTaskToDb()
                    action();
                } 
                .padding(.top, 60)
...
#Preview {
    AddTaskView(task:TaskModel(category: Category.personal.rawValue)) {
